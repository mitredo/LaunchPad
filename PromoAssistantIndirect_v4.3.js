...
$(document).ready(function () {
	...
	for(var i = 0; i<12; i++){   // 8 represents 9 indices
    ...
    for(i = 0; i < promosArray.length;i++) {
		...
		var walmartLocationEmployeeImpact = item.walmartLocationEmployeeImpact;
		var bestBuyLocationEmployeeImpact = item.bestBuyLocationEmployeeImpact;
		...
		for (var j in channels) {
			...
			if (channels[j] == 'Indirect National Retail')
			{
				for (var k=0; k<nationalRetailers.length; k++)
				{
					...
					// done in direct array
						//pushElements(5,item,promotiontype,oems);
						pushElements(6,item,promotiontype,oems);

						for(var count = 0; count < bestBuyLocationEmployeeImpact.length; count++){
							if(bestBuyLocationEmployeeImpact[count]=="Best Buy Employee, Core Blue POS"){
								pushElements(7,item,promotiontype,oems);
							}
							// done in other array with direct links
							// if(bestBuyLocationEmployeeImpact[count]=="Best Buy Employee, Omni NR"){
							// 	pushElements(8,item,promotiontype,oems);
							// }
							// if(bestBuyLocationEmployeeImpact[count]=="Verizon Employee, Omni NR"){
							// 	pushElements(9,item,promotiontype,oems);
							// }
						}

						for(var count = 0; count < walmartLocationEmployeeImpact.length; count++){
							if(walmartLocationEmployeeImpact[count]=="Walmart Employee"){
								pushElements(10,item,promotiontype,oems);
							}
							// done in other array with direct links
							// if(walmartLocationEmployeeImpact[count]=="Walmart Exclusive Labor"){
							// 	pushElements(11,item,promotiontype,oems);
							// }
						}
						break;
					}
	...
	// Now we do direct national retailer links
	...
	$(".top_level").on("click", function() {
	 	...
	 	switch (selectedChannel) {
			...
			case "BestBuyVZEOmni":
				i = 9;
				break;
			case "WalmartEmployee":
				i = 10;
				break;	
			case "WalmartExclusive":
				i = 11;
				break;
...
function cbChange(obj) {
	...
	else if (selectedChannel == "BestBuyPos")
	{
		$('#tab__header-9').hide();
		reloadGrids(7);	
	}
	else if (selectedChannel == "BestBuyOmni")
	{
		$('#tab__header-9').hide();
		reloadGrids(8);	
	}
	else if (selectedChannel == "BestBuyVZEOmni")
	{
		$('#tab__header-9').hide();
		reloadGrids(9);	
	}
	else if (selectedChannel == "WalmartEmployee")
	{
		$('#tab__header-9').hide();
		reloadGrids(10);	
	}
	else if (selectedChannel == "WalmartExclusive")
	{
		$('#tab__header-9').hide();
		reloadGrids(11);	
	}
