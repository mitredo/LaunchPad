...
function setColModel()
{
	...
	colModel.push({label: 'Key Points', name: 'keypoints',formatter:formatkeypoints, width: 470,align: 'left', sortable:false });
...
function formatdate(cellValue, options, rowObject) {
	// launchDate is not in Moment format here because Pushelement did not push the formated date
	var cellHtml = "";
	var today = moment(new Date());
	var formatToday = today.format('YYYY-MM-DD');
	var launchDate = moment(new Date (rowObject.launchdate));
	...
	var endDate = moment(new Date (rowObject.enddate));
...
function pushElements(i,element,promotiontype,oems)
{
	...
	// write to the All bucket regardless - do not need for Loyalty
