			...
			if (channels[j] == 'Indirect National Retail')
			{
				for (var k=0; k<nationalRetailers.length; k++)
				{
					if (nationalRetailers[k]=='All Retailers')
					{
						...
						pushElements(4,element,promotiontype,oems);
						pushElements(5,element,promotiontype,oems);
						pushElements(6,element,promotiontype,oems);
						break;
					}
...
function setColModel()
{
	...
	colModel.push({label: 'Promotion/OST Link<br>(Verizon log-in required)',name:'project',width: 130, align: 'left', sortable:true});
